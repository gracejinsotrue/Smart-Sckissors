...
public class ScissorsSelectionModel extends SelectionModel {
    ...
    protected void appendToSelection(Point p) {
        ...
        List <Integer> shortestPaths = paths.pathTo(vertexId);
        // 3. convert path
        PolyLine polyline = graph.pathToPolyLine(shortestPaths);
        //append said path to our selection
        selection.addLast(polyline);
        //call findpaths() to start a new shortest path from our selections new endpoint
        findPaths(vertexId);
    ...
    public PolyLine liveWire(Point p) {
        ...
        int vertexId = graph.idAt(p); // step 1

        List <Integer> shortestPaths = paths.pathTo(vertexId); // this line may be wrong
        PolyLine polyline = graph.pathToPolyLine(shortestPaths);
    ...
    private class ShortestPathsWorker
        ...
        public PathfindingSnapshot doInBackground() {
            ...
            while(!pathfinder.allPathsFound()){
                ...
                result = pathfinder.extendSearch(10000);

             
                int totalvertices= pathfinder.vertexCount();
                int percentage = (int) ((double) pathfinder.settledCount()/totalvertices*100);
        ...
        public void done() {
            ...
            if (!worker.equals(this)) { return; }

            try {
                paths = get();
               
                if (previousState == NO_SELECTION) {
                    previousState = SELECTING;
                }
            }
            //step 4
