...
public class ShortestPaths<VertexType extends Vertex<EdgeType>, EdgeType extends Edge> {
    ...
    public PathfindingSnapshot extendSearch(int maxToSettle) {
        ...
        int settledCount = 0;
        while (settledCount < maxToSettle && !allPathsFound()) {

            int currentVertexId = frontier.remove();
            // the id of the vertex we want to find the shortest path to
            VertexType currentVertex = graph.getVertex(currentVertexId); 

            // connect the current vertex to all it's known outgoing edges
            for (EdgeType edge : currentVertex.outgoingEdges()) { 
                int connectedEdgeId = edge.endId();
                if (!settledIds.get(connectedEdgeId)) {
                    int connectedDistance = distances[currentVertexId] + weigher.weight(edge);
                    int knownDistance = distances[connectedEdgeId];

                    if (knownDistance == -1 || connectedDistance < knownDistance) {
                        distances[connectedEdgeId] = connectedDistance;
                        predecessors[connectedEdgeId] = currentVertexId;
                        frontier.addOrUpdate(connectedEdgeId, connectedDistance);
                    }
                }
            }

            settledIds.set(currentVertexId);
            settledCount++;
