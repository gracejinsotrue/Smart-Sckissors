...
public abstract class SelectionModel {
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        if (state != SELECTED) {
            throw new IllegalStateException("Selection must be finished to find closest point");
        }

        int closestIndex = -1;
        int closestDistanceSq = maxDistanceSq + 1;
        ...
        for (int i = 0; i < selection.size(); i++) {
            ...
            //square of max distance
            int distanceSq = (startPoint.x - p.x) * (startPoint.x - p.x);
            distanceSq = distanceSq + (startPoint.y - p.y) * (startPoint.y - p.y);

            //check if squared dist is within max dist
