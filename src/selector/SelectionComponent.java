...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintSelectionPerimeter(Graphics g, List<PolyLine> segments) {
        ...
        int[] xPoints = new int[segments.size() + 1];
        int[] yPoints = new int[segments.size() + 1];
        int nPoints = segments.size() + 1;

        for (int i = 0; i < segments.size(); i++) {
            PolyLine segment = segments.get(i);

            Point startPoint = segment.start();
            int xcoord = startPoint.x;
            int ycoord = startPoint.y;

            xPoints[i] = xcoord;
            yPoints[i] = ycoord;
        }

        if (!segments.isEmpty()) { // fixme: This may be unreliable code
            xPoints[segments.size()] = segments.get(segments.size() - 1).end().x;
            yPoints[segments.size()] = segments.get(segments.size() - 1).end().y;
    ...
    private void paintLiveWire(Graphics g) {
        ...
        Point start = model.lastPoint().getLocation();

        g.drawLine(start.x, start.y, mouseLocation.x, mouseLocation.y);
    }
    ...
    private void paintControlPoints(Graphics g, List<PolyLine> segments) {
        ...
        //filled circle at start and end
        g.setColor(controlPointColor);
        for (PolyLine segment : segments) {
            Point start = segment.start();
            Point end = segment.end();

            //filled circle at start point
            g.fillOval(start.x - controlPointRadius, start.y - controlPointRadius,
                    2 * controlPointRadius, 2 * controlPointRadius);

            //filled circle at end
            g.fillOval(end.x - controlPointRadius, end.y - controlPointRadius,
                    2 * controlPointRadius, 2 * controlPointRadius);
    ...
    private void paintMoveGuides(Graphics g, List<PolyLine> segments) {
        ...
        if (selectedIndex != 0) {
            startPoint = segments.get(selectedIndex - 1).start();
        }
        else {
            startPoint = segments.get(segments.size() - 1).start();
        }
        Point endPoint = segments.get(selectedIndex).end();

        g.drawLine(startPoint.x, startPoint.y, mouseLocation.x, mouseLocation.y);
        g.drawLine(endPoint.x, endPoint.y, mouseLocation.x, mouseLocation.y);
    }
    ...
    public void mousePressed(MouseEvent e) {
         ...
         if(e.getButton() == MouseEvent.BUTTON1&&model.state()==SELECTED) {
             ...
             int closestIndex = model.closestPoint(position,
